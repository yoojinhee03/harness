...
class AppSettingsStore:
    ...
    def resolve(self) -> dict[str, Any]:
        """내부용 — 복호한 키 포함. provider/llm_key/embedding_key."""
        r = self._row()
        if r is None:
            return {"provider": "anthropic", "llm_key": "", "embedding_key": ""}
        return {
            ...
            if existing is None:
                conn.execute(
                    insert(app_settings).values(
                        id=_ROW_ID, provider=prov, llm_key_enc=llm_enc, embedding_key_enc=emb_enc, updated_at=ts
                    )
                )
            else:
                conn.execute(
                    ...
                    .values(provider=prov, llm_key_enc=llm_enc, embedding_key_enc=emb_enc, updated_at=ts)
                )
        return self.status()
