...
export function CommandPalette({
  ...
  const cmds = useMemo<Cmd[]>(
    () => [
      ...
      { id: "go-create", label: "이동: 생성", run: () => { setView("create"); onClose(); } },
      { id: "go-catalog", label: "이동: 카탈로그", run: () => { setView("catalog"); onClose(); } },
      { id: "go-harnesses", label: "이동: 하네스", run: () => { setView("harnesses"); onClose(); } },
      { id: "go-settings", label: "이동: 설정", run: () => { setView("settings"); onClose(); } },
      { id: "theme", label: "테마 전환 (다크/라이트)", run: () => { toggleTheme(); onClose(); } },
    ],
