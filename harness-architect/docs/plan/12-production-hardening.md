...
- **팀 역할 UI 완료(2026-09-07)** — 멤버 목록에서 역할 변경·제거·자진 탈퇴.
  구현 중 **권한 상승 구멍**을 발견해 함께 막았다: `add_member` 가 기존 멤버를 만나면 무조건 역할을
  UPDATE 했고 actor 조건이 owner/editor 라서, **editor 가 자기 이메일로 `role=owner` 를 보내 스스로
  승격**할 수 있었다. 정책이 owner 전용이 된 뒤로는 그게 곧 거버넌스 우회였다(403 → 승격 → 200).
  이제 역할 변경·owner 부여는 owner 만이고, **owner 최소 1명**을 불변식으로 지킨다(0 이 되면 멤버
  관리도 정책 변경도 아무도 못 하는 통치 불가 상태).
- **남은 것(외부 결정)** — 배포 타깃(호스트) 선택 + 프로덕션 시크릿 주입.
